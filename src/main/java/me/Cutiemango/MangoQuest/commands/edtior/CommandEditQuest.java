			...
			switch (t) {
				...
				case ALLOW_DESCENDANT:
					EditorListenerHandler.register(sender, new EditorListenerObject(ListeningType.STRING, "mq e edit req " + t.toString(),
							Syntax.of("B", I18n.locMsg("Syntax.Boolean"))));
					QuestBookGUIManager.openInfo(sender, I18n.locMsg("EditorMessage.EnterValue"));
					break;
				case FRIEND_POINT:
					...
					EditorListenerHandler.register(sender, new EditorListenerObject(ListeningType.STRING, "mq e edit req FRIEND_POINT",
							Syntax.of("N:D", I18n.locMsg("Syntax.FriendPoint"), ":")));
					break;
				case ITEM:
	...
	private static void editNPC(Quest q, Player sender, String[] args) {
		if (args.length == 3) {
			EditorListenerHandler.register(sender,
					new EditorListenerObject(ListeningType.NPC_LEFT_CLICK, "mq e edit npc", Syntax.of("N", I18n.locMsg("Syntax.NPCID"), "")));
		...
		switch (args.length) {
			...
			case 6:
				...
				if (!(qobj instanceof EditorObject))
					return;
				EditorListenerObject eobj = ((EditorObject) qobj)
						.createCommandOutput(sender, "mq e edit object " + stage + " " + obj + " " + args[5], args[5]);
				if (eobj == null)
					return;
		...
		if (args.length == 4) {
			switch (args[3].toLowerCase()) {
				...
				case "qrpgexp":
					...
					break;
				case "npc":
					EditorListenerHandler.register(sender, new EditorListenerObject(ListeningType.NPC_LEFT_CLICK, "mq e edit reward " + args[3],
							Syntax.of("N", I18n.locMsg("Syntax.NPCID"), "")));
					QuestBookGUIManager.openInfo(sender, I18n.locMsg("EditorMessage.ClickNPC"));
					return;
				case "item":
				...
				case "fp":
					QuestBookGUIManager.openInfo(sender, I18n.locMsg("EditorMessage.FriendPoint"));
					EditorListenerHandler.register(sender, new EditorListenerObject(ListeningType.STRING, "mq e edit reward fp",
							Syntax.of("N:D", I18n.locMsg("Syntax.FriendPoint"), ":")));
					return;
				default:
					QuestEditorManager.editQuest(sender);
