...
public class QuestObjectKillMob extends NumerableObject implements EditorObject
{
	...
	public QuestObjectKillMob(MythicMob mmMob, int i) {
		mtmMob = mmMob;
		amount = i;
		type = EntityType.valueOf(mmMob.getEntityType().name().toUpperCase());
	...
	public void setMythicMob(MythicMob m) {
		...
		if (mtmMob.getEntityType().name().contains("BABY"))
			isBaby = true;
		type = EntityType.valueOf(mtmMob.getEntityType().name().replace("BABY_", "").toUpperCase());
	}
	...
	public boolean load(QuestIO config, String path) {
		if (config.getString(path + "MythicMob") != null) {
			...
			String typeName = mtmMob.getEntityType().name().toUpperCase();
	...
	public boolean receiveCommandInput(Player sender, String type, String obj) {
		switch (type) {
			...
			case "mtmmob":
				...
				if (mtmMob.getEntityType().name().contains("baby")) {
					isBaby = true;
					setType(EntityType.valueOf(mtmMob.getEntityType().name().replace("baby_", "").toUpperCase()));
				} else
					setType(EntityType.valueOf(mtmMob.getEntityType().name().toUpperCase()));
				break;
			default:
